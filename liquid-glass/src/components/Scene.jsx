...
const NEON_TUBES = [
  {
    id: 'cyan',
    points: [[-15, 2, 0], [-8, 4, 1], [0, 2.5, 0], [8, 4, -1], [15, 2, 0]],
    color: '#00e5ff',
    radius: 0.035,
    intensity: 1
  },
  {
    id: 'purple',
    points: [[15, -1, 0], [8, 0.5, 1], [0, -1.5, 0], [-8, 0, -1], [-15, -2, 0]],
    color: '#a855f7',
    radius: 0.03,
    intensity: 0.9
  },
  {
    id: 'pink',
    points: [[-12, -6, 1], [-4, -2, 0], [4, 2, -1], [12, 6, 0]],
    color: '#ec4899',
    radius: 0.03,
    intensity: 0.8
  }
...
function NeonTube({ points, color, radius = 0.03, intensity = 1 }) {
  ...
  const glowGeometry = useMemo(
    () => new THREE.TubeGeometry(curve, 64, radius * 3, 8, false),
    [curve, radius]
  )
...
function Scene({
  ...
  useFrame(({ mouse }, delta) => {
    ...
    if (groupRef.current && animationProgressRef.current < 1) {
      animationProgressRef.current = Math.min(1, animationProgressRef.current + frameDelta * 1.2)
    ...
    if (groupRef.current && !isMobile) {
      ...
      const smoothing = 1 - Math.exp(-frameDelta * 1.52)
      ...
      const smoothing = 1 - Math.exp(-frameDelta * 5)
      groupRef.current.rotation.y = THREE.MathUtils.lerp(groupRef.current.rotation.y, -mouse.x * 0.08, smoothing)
      groupRef.current.rotation.x = THREE.MathUtils.lerp(groupRef.current.rotation.x, mouse.y * 0.04, smoothing)
    }
  })
  ...
  return (
    <>
      ...
      <group position={[0, 0, -4]}>
        {NEON_TUBES.map((tube) => <NeonTube key={tube.id} {...tube} />)}
      </group>
