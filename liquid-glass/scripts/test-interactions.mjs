...
assert.equal(normalizeRealityStoryIndex(-1, 4), 3)
...
const realityStyles = readFileSync(new URL('../src/components/styles/RealityLabView.css', import.meta.url), 'utf8')
...
assert.match(realityView, /memory-cutout\.webp[\s\S]+memento-cutout\.webp[\s\S]+heritage-cutout\.webp[\s\S]+aitop-cutout\.webp/)
assert.match(realityView, /onPointerMove=\{updateHeroDrag\}/)
assert.match(realityView, /proofSection:\s*'awards'/)
assert.match(realityView, /reality-transition-wash/)
assert.doesNotMatch(realityView, /StoryImage|reality-object-card|reality-hero-controls|CULTURE \/ RESEARCH/)
assert.match(realityStyles, /\.reality-hero-slide\.slot-active/)
assert.match(realityStyles, /\.reality-cinematic-shell,[\s\S]+position:\s*absolute;[\s\S]+inset:\s*0;/)
assert.match(realityStyles, /\.reality-proof-tabs button span\s*\{[\s\S]+font-size:\s*1\.25rem;/)
assert.equal(realityManifest.generator, 'Vertex AI Gemini API')
assert.equal(realityManifest.model, 'gemini-3-pro-image-preview')
assert.equal(realityManifest.image_size, '2K')
assert.equal(realityManifest.semantic_version, 4)
assert.deepEqual(Object.keys(realityManifest.prompts.stories).sort(), ['aitop', 'heritage', 'memento', 'memory'])
assert.doesNotMatch(JSON.stringify(realityManifest.prompts), /glass cube/i)
assert.equal(Object.keys(realityManifest.assets).length, 33)
for (const storyId of ['memory', 'memento', 'heritage', 'aitop']) {
  for (const kind of ['real', 'sketch', 'cutout', 'scene']) {
    assert.equal(`${storyId}-${kind}.webp` in realityManifest.assets, true)
  }
}
