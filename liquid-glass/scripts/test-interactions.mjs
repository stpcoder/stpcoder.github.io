...
assert.match(realityView, /Sketch[\s\S]+Resolve[\s\S]+Evidence[\s\S]+In context/)
assert.match(realityView, /onPointerMove=\{updateCardRevealFromPointer\}/)
assert.match(realityView, /DRAM package → mobile device/)
assert.match(realityView, /Travel photo → 3D collectible/)
assert.match(realityView, /Damaged painting → conserved panel/)
assert.doesNotMatch(realityView, /siliconSketch|intelligenceSketch|realityRoom/)
assert.match(realityStyles, /\.reality-object-card\.is-left[\s\S]+\.reality-object-card\.is-right/)
assert.match(realityStyles, /data-step='evidence'[\s\S]+data-step='context'/)
assert.equal(realityManifest.generator, 'Vertex AI Gemini API')
assert.equal(realityManifest.model, 'gemini-2.5-flash-image')
assert.equal(realityManifest.semantic_version, 2)
assert.deepEqual(Object.keys(realityManifest.prompts.stories).sort(), ['heritage', 'memento', 'memory'])
assert.doesNotMatch(JSON.stringify(realityManifest.prompts), /glass cube|silicon wafer/i)
assert.deepEqual(
  Object.keys(realityManifest.assets).sort(),
  ['heritage-real.webp', 'heritage-scene.webp', 'heritage-sketch.webp', 'memento-real.webp', 'memento-scene.webp', 'memento-sketch.webp', 'memory-real.webp', 'memory-scene.webp', 'memory-sketch.webp']
)
